...
class FlashDockRegistryClient:
    ...
    def discover_compass_services(self, healthy_only: bool = True) -> List[ServiceInfo]:
        ...
        try:
            ...
            services = self.client.discover_services(
                service_name="compass-service", status_filter=status_filter
            )
            logger.info(f"Discovered {len(services)} COMPASS service(s)")
            return services
        except requests.exceptions.Timeout as e:
            logger.error(
                f"Timeout while discovering COMPASS services (timeout: {self.timeout}s): {e}"
            )
    ...
    def check_registry_available(self) -> bool:
        ...
        try:
            logger.debug(
                f"Checking registry health at {self.registry_url} (timeout: {self.timeout}s)"
            )
