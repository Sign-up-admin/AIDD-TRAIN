...
with tab1:
    ...
    with st.form("create_task_form"):
        ...
        try:
            ...
            dataset_options = ["无（使用默认数据集）"] + [f"{ds['dataset_id']} - {ds['name']}" for ds in datasets if ds.get('status') == 'ready']
    ...
    try:
        ...
        if tasks:
            ...
            for task in tasks:
                ...
                total_epochs = progress_info.get('total_epochs', task.get('config', {}).get('epochs', 0))
                ...
                display_tasks.append({
                    ...
                    "轮数": f"{current_epoch}/{total_epochs}" if total_epochs > 0 else f"{task.get('config', {}).get('epochs', 'N/A')}",
            ...
            if selected_task_id:
                ...
                if selected_task:
                    ...
                    with col4:
                        if st.button("查看详情", use_container_width=True, key="view_details_quick"):
        ...
        try:
            ...
            with col2:
                if task.get('created_at'):
                    st.metric("创建时间", task['created_at'][:19] if len(task['created_at']) > 19 else task['created_at'])
                if task.get('started_at'):
                    st.metric("开始时间", task['started_at'][:19] if len(task['started_at']) > 19 else task['started_at'])
            ...
            if progress:
                ...
                if total_epochs > 0:
                    ...
                    st.write(f"当前轮数: {current_epoch} / {total_epochs} ({progress_percent:.1f}%)")
            ...
            with col2:
                log_limit = st.number_input("日志行数", min_value=10, max_value=1000, value=100, step=10, key="log_limit")
