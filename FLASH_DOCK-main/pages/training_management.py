...
with tab1:
    ...
    if st.session_state.get("show_task_actions", False) and st.session_state.get("last_created_task"):
                        ...
                        if task_status == "running" or task_status == "initializing":
                            if st.button("停止任务", key="stop_quick"):
                                ...
                                with st.spinner("正在停止任务..."):
                                    ...
                                    try:
                                        # 直接发送停止请求
                                        result = client.stop_training_task(selected_task_id)
                                        ...
                                        if not result or not isinstance(result, dict):
                                            ...
                                            # 尝试获取任务状态以确认
                                            try:
                                                latest_task = client.get_training_task(selected_task_id)
                                        ...
                                        if 'stopped' not in response_message.lower() and 'stop' not in response_message.lower():
                                            st.warning("⚠️ 后端响应消息异常，无法确认停止请求是否被接受")
                                        
                                        # 立即检查任务状态，确认是否真的停止成功
                                        status_checked = False
                                        try:
                                            updated_task = client.get_training_task(selected_task_id)
                                            ...
                                            if current_status in ['cancelled', 'completed', 'failed']:
                                                ...
                                                st.success(f"✓ 任务已成功停止，当前状态: {status_messages.get(current_status, current_status)}")
                                                status_checked = True
                                                time.sleep(1)
                                                st.rerun()
                                        ...
                                        if not status_checked:
                                            ...
                                            # 轮询检查任务状态
                                            max_poll_time = 30.0  # 增加到30秒
                                            poll_interval = 1.0  # 增加到1秒间隔
                                            poll_elapsed = 0.0
                                            final_status = None
                                            ...
                                            while poll_elapsed < max_poll_time:
                                                try:
                                                    ...
                                                    status_placeholder.info(f"正在检查任务状态... ({poll_elapsed:.1f}s / {max_poll_time:.0f}s) - 当前状态: {final_status}")
                                                    ...
                                                    if final_status in ['cancelled', 'completed', 'failed']:
                                                        ...
                                                        status_placeholder.success(f"✓ 任务已成功停止，当前状态: {status_messages.get(final_status, final_status)}")
                                                        break
                                                    elif final_status not in ['running', 'initializing']:
                                                        progress_bar.progress(1.0)
                                                        status_placeholder.warning(f"⚠️ 任务状态已改变为: {final_status}，但非预期的停止状态")
                                                        break
                                                except Exception as poll_e:
                                                    # 轮询时出错，继续轮询
                                                    status_placeholder.warning(f"⚠️ 轮询时出错: {str(poll_e)[:50]}... (继续轮询)")
                                            ...
                                            if final_status and final_status in ['cancelled', 'completed', 'failed']:
                                                ...
                                                status_placeholder.warning("⚠️ 停止请求已发送，但任务仍在运行。任务可能正在停止中，请稍后刷新页面查看最新状态。")
                                            else:
                                                status_placeholder.warning(f"⚠️ 无法确认任务停止状态。当前状态: {final_status if final_status else '未知'}")
...
with tab3:
    ...
    if task_id_input:
        try:
                ...
                if task_status == "running" or task_status == "initializing":
                    if st.button("停止任务", key="stop_detail"):
                        ...
                        with st.spinner("正在停止任务..."):
                            ...
                            try:
                                # 直接发送停止请求
                                result = client.stop_training_task(task_id_input)
                                stop_success = True
                                
                                # 验证响应
                                if result and isinstance(result, dict):
                                    if "task_id" in result:
                                        st.success(f"✓ 任务停止请求已发送 (任务ID: {result.get('task_id', task_id_input)})")
                                    else:
                                        st.success("✓ 任务停止请求已发送")
                                else:
                                    st.warning("⚠️ 停止请求已发送，但响应格式异常")
                                
                                # 等待一下让用户看到成功消息
                                time.sleep(0.5)
                                
                                # 轮询检查任务状态，延长等待时间以提高成功率
                                max_poll_time = 30.0  # 增加到30秒
                                poll_interval = 1.0  # 增加到1秒间隔
                                poll_elapsed = 0.0
                                final_status = None
                                ...
                                while poll_elapsed < max_poll_time:
                                    try:
                                        updated_task = client.get_training_task(task_id_input)
                                        final_status = updated_task.get('status')
                                        ...
                                        if final_status in ['cancelled', 'completed', 'failed']:
                                            ...
                                            progress_bar.progress(1.0)
                                            status_placeholder.success(f"✓ 任务状态已更新为: {status_messages.get(final_status, final_status)}")
                                            break
                                        elif final_status not in ['running', 'initializing']:
                                            progress_bar.progress(1.0)
                                            status_placeholder.warning(f"⚠️ 任务状态: {final_status} (非预期状态)")
                                            break
                                    except Exception as poll_e:
                                        # 轮询时出错，但停止请求已发送，继续轮询
                                        # 不中断轮询，避免停止请求已发送但前端停止监控
                                        status_placeholder.warning(f"⚠️ 轮询时出错: {str(poll_e)[:50]}... (继续轮询)")
                                ...
                                if final_status and final_status in ['running', 'initializing']:
                                    status_placeholder.warning("⚠️ 停止请求已发送，但任务状态仍未更新。任务可能正在停止中，请稍后刷新页面查看最新状态。")
                                ...
                                if e.status_code == 400:
                                    ...
                                    # 尝试获取最新状态
                                    try:
                                        latest_task = client.get_training_task(task_id_input)
                                        latest_status = latest_task.get('status')
                                        st.info(f"📊 当前任务状态: {latest_status}")
                                    except:
                                        pass
            ...
            """
            components.html(error_listener_script, height=0, key=f"error_listener_{task_id_input}")
            ...
            use_realtime = st.checkbox(
                ...
                value=st.session_state.get(
                    f"realtime_{task_id_input}", task_status_for_display in ["running", "initializing"]
                ),
                ...
                if task_status_for_display not in valid_statuses:
                    ...
                    if ws_url:
                        ...
                        components.html(
                            ...
                            height=600,
                            scrolling=False,
                            key=f"terminal_component_{task_id_input}",
                        )
                        
                        # Add refresh button for terminal
                        col_refresh1, col_refresh2 = st.columns([4, 1])
                        with col_refresh2:
                            if st.button("🔄 刷新终端", key=f"refresh_terminal_pending_{task_id_input}", help="刷新终端连接"):
                                st.rerun()
                else:
                    ...
                    if ws_url:
                        ...
                        col_refresh1, col_refresh2 = st.columns([4, 1])
                        with col_refresh2:
                            if st.button("🔄 刷新终端", key=f"refresh_terminal_running_{task_id_input}", help="刷新终端连接"):
                                st.rerun()
