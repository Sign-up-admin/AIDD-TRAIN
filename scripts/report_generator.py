...
class ReportGenerator:
    ...
    def generate_frontend_error_report(self, debug_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"- **Total errors:** {debug_results.get('summary', {}).get('total_errors', 0)}\n\n"
        )
            ...
            if result.get("status") == "error" or result.get("errors"):
                ...
                if console_logs:
                    ...
                    for log in console_logs:
                        report.append(
                            f"- **{log.get('type', 'unknown')}:** {log.get('text', 'N/A')}\n"
                        )
                        report.append(
                            f"  - **Location:** {log.get('location', {}).get('url', 'N/A')}\n\n"
                        )
    ...
    def generate_backend_error_report(self, debug_results: Dict[str, Any]) -> str:
        ...
        for result in api_results:
            if result.get("status") >= 400 or result.get("error"):
                report.append(
                    f"### {result.get('method', 'UNKNOWN')} {result.get('path', 'N/A')}\n\n"
                )
        ...
        if slow_requests:
            ...
            for result in slow_requests:
                report.append(
                    f"- **{result.get('method', 'UNKNOWN')} {result.get('path', 'N/A')}:** {result.get('response_time', 0):.2f}s\n"
                )
    ...
    def generate_comprehensive_report(
        ...
        report.append(
            f"- **Total network requests:** {debug_summary.get('total_network_requests', 0)}\n\n"
        )
        ...
        if high_severity_vulns:
            ...
            for vuln in high_severity_vulns:
                report.append(
                    f"1. **{vuln.get('title', 'Unknown')}** - {vuln.get('category', 'unknown')}\n"
                )
                report.append(f"   - Location: {vuln.get('location', 'N/A')}\n")
                report.append(f"   - Recommendation: {vuln.get('recommendation', 'N/A')}\n\n")
