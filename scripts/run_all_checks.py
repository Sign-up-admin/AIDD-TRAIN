...
def find_conda_python(env_name: str) -> Optional[str]:
    ...
    try:
        result = subprocess.run(
            ["conda", "info", "--base"], capture_output=True, text=True, timeout=5
        )
...
def ensure_conda_environment() -> Tuple[bool, Optional[str]]:
    ...
    for env_name in env_names:
        ...
        if python_path:
            ...
            print("[INFO] Will use conda environment Python for running checks.")
...
def main():
    ...
    if args.no_tests:
        ...
        if args.tool == "pytest":
            print(
                "[ERROR] Cannot use --tool pytest with --no-tests. These options are conflicting."
            )
            sys.exit(1)
    ...
    if missing_tools:
        ...
        if args.ci or not is_interactive:
            ...
            if result.returncode != 0:
                print("[ERROR] Failed to install missing tools. Please install manually:")
            ...
            try:
                ...
                if response.lower() == "y":
                    subprocess.run(
                        [sys.executable, "-m", "pip", "install", "-q"] + missing_tools, check=False
                    )
