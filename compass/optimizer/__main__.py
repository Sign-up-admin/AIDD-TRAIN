...
def find_optimal_configs(modes_to_optimize, current_dataset_size, project_root_path):
    ...
    try:
        for mode in modes_to_run:
            ...
            if mode == 'production':
                optimizer_args = (param_combinations, max_params, start_bs, stress_iters, processed_test_data)
            elif mode in ['validation', 'prototyping']:
                optimizer_args = (mode, num_layers, hidden_channels, max_params, start_bs, stress_iters, processed_test_data)
            else: # smoke_test
