...
try:
    ...
    print(
        "ERROR: scikit-optimize library not found. Please install it with: pip install scikit-optimize"
    )
    sys.exit(1)
...
def _optimize_for_production(
    param_combinations, max_params, start_bs, stress_iters, processed_test_data
):
    """
    ...
    for i, (num_layers, hidden_channels) in enumerate(param_combinations):
        logger.info(
            f"> Probing Arch {(i+1)}/{len(param_combinations)} (L={num_layers}, C={hidden_channels})..."
        )
        num_params = sum(
            p.numel()
            for p in ViSNetPDB(hidden_channels=hidden_channels, num_layers=num_layers).parameters()
            if p.requires_grad
        )
        if num_params > max_params:
            logger.info(f"SKIPPED (too complex: {num_params/1e3:.0f}k > {max_params/1e3:.0f}k cap)")
            continue
        base_config = {"visnet_hidden_channels": hidden_channels, "visnet_num_layers": num_layers}
        success, _ = probe_config(
            {**base_config, "batch_size": 1}, processed_test_data, stress_iterations=5
        )
        if not success:
            logger.info("SKIPPED (too large for VRAM)")
            continue
    ...
    if best_architecture:
        ...
        optimal_bs, _ = find_max_batch_size_by_stressing(
            best_architecture, processed_test_data, start_bs, stress_iters
        )
        if optimal_bs:
            return {**best_architecture, "batch_size": optimal_bs}
    logger.warning("Could not find any suitable model architecture.")
    return None


def _optimize_for_efficiency_modes(
    mode,
    num_layers_list,
    ...
    logger.info(
        f"--- Strategy: Bayesian Optimization to find most EFFICIENT config in 'sweet spot' ({min_time}-{max_time} mins) for '{mode}' mode. ---"
    )
    ...
    def evaluate_config(params):
        ...
        num_params = sum(
            p.numel()
            for p in ViSNetPDB(hidden_channels=channels, num_layers=layers).parameters()
            if p.requires_grad
        )
        if num_params > max_params:
            logger.info(f"> SKIPPED: Model has {num_params/1e3:.1f}k params, exceeding cap.")
            return 0.0
        ...
        found_bs, avg_time_per_iter = find_max_batch_size_by_stressing(
            base_config, processed_test_data, start_bs, stress_iters
        )
        if not found_bs:
            logger.warning("> Could not find a stable batch size for this architecture.")
            return 0.0
        ...
        if eval_estimated_time > max_time * 1.2:  # Allow some leeway
            logger.info(f"> PRUNING: Estimated time ({eval_estimated_time:.2f}m) too high.")
            return 0.0
    ...
    for eval_data in evaluation_cache.values():
        ...
        if min_time <= estimated_time <= max_time:
            if efficiency_score > best_efficiency_in_sweet_spot:
                best_efficiency_in_sweet_spot, best_in_sweet_spot = (
                    efficiency_score,
                    candidate_config,
                )
                logger.info(
                    f"> NEW BEST in '{mode}' sweet spot: BS={cfg_optimal_bs}, L={cfg_layers}, C={cfg_channels} (Time: {estimated_time:.2f}m, Score: {efficiency_score:.2f})"
                )
    ...
    if best_overall:
        logger.info(
            "--- Final Decision: No config hit the 'sweet spot'. Falling back to best overall efficiency. ---"
        )
        return best_overall
...
def _optimize_for_smoke_test(param_combinations, max_params, processed_test_data):
    ...
    if num_params > max_params:
        logger.warning(
            f"Smallest model ({num_params/1e3:.1f}k params) exceeds data cap ({max_params/1e3:.0f}k)."
        )
    base_config = {
        "visnet_hidden_channels": hidden_channels,
        "visnet_num_layers": num_layers,
        "batch_size": 1,
    }
    logger.info(
        f"--- Trying smallest model (L={num_layers}, C={hidden_channels}, BS=1) for a single iteration ---"
    )
