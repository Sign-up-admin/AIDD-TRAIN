...
def prepare_real_data(project_root_path):
    ...
    protein_path = os.path.join(
        project_root_path, "compass", "optimizer", "1jmf", "1jmf_pocket.pdb"
    )
    ...
    logger.info(
        f"Temporarily setting max_atoms to {max_atoms_for_sample} to process {sample_name}..."
    )
    ...
    if processed_test_data is None:
        logger.critical(f"Failed to process {sample_name} data. Aborting.")
        exit()

    logger.info(
        f"--- Successfully processed {sample_name}. Atom count: {processed_test_data.num_nodes} ---"
    )
