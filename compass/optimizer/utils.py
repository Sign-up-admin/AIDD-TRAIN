...
def load_optimized_settings(config):
    ...
    if not mode_settings:
        logger.info(
            f"No optimized settings found for '{mode}' mode in 'hardware_profile.json'. Using default settings."
        )
        return

    logger.info(
        f"Applying hardware-optimized settings from 'hardware_profile.json' for '{mode}' mode."
    )
