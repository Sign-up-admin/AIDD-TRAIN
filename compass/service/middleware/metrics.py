...
class MetricsCollector:
    ...
    def get_metrics(self) -> Dict:
        ...
        avg_response_time = sum(request_times_list) / len(request_times_list) if request_times_list else 0
        ...
        avg_response_size = sum(response_sizes_list) / len(response_sizes_list) if response_sizes_list else 0
...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path in ["/health", "/health/ready", "/docs", "/openapi.json", "/redoc", "/metrics"]:
