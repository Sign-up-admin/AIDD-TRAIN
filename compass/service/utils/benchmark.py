...
class BenchmarkRunner:
    ...
    def run_benchmark(
        ...
        **kwargs
        ...
        p95_time = sorted_times[int(len(sorted_times) * 0.95)] if len(sorted_times) > 1 else times[0]
        p99_time = sorted_times[int(len(sorted_times) * 0.99)] if len(sorted_times) > 1 else times[0]
    ...
    def print_summary(self):
        ...
        for result in self.results:
            print(f"\n{result.name}")
            print(f"  Iterations: {result.iterations} (Success: {result.success_count}, Errors: {result.error_count})")
