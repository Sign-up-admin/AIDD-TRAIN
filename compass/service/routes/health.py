...
async def run_benchmark(benchmark_type: str = "inference", iterations: int = 10):
    """
    ...
    if benchmark_type == "inference" or benchmark_type == "all":
        try:
            ...
            result = benchmark_inference_service(inference_service, num_iterations=iterations)
...
async def get_alerts(level: Optional[str] = None, limit: int = 100):
    ...
    return {"alerts": alert_manager.get_alerts(level=alert_level, limit=limit)}


@router.get(
    "/alerts/summary", summary="Get Alert Summary", description="Get alert summary statistics"
)
