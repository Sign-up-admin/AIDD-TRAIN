...
def check_zip_bomb(file_path: str, max_ratio: int = MAX_COMPRESSION_RATIO, max_files: int = MAX_FILES_IN_ARCHIVE) -> bool:
    """
...
@router.post("/api/v1/data/upload", response_model=DatasetResponse, status_code=status.HTTP_201_CREATED)
async def upload_dataset(
    ...
    dataset_id = data_service.create_dataset(
        name=dataset_name,
    ...
    try:
        ...
        upload_task = upload_queue.submit_upload(
            upload_task_id,
            dataset_id,
            upload_func
        )
