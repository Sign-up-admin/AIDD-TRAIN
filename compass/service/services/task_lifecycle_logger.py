...
class TaskLifecycleLogger:
    ...
    def _write_lifecycle_event(self, event_type: str, stage: str, status: str, 
    ...
    def log_timeout(self, stage: str, timeout_seconds: float):
        ...
        self.logger.warning(f"Task {self.task_id} timed out in stage {stage} after {timeout_seconds}s")
    ...
    def log_state_transition(self, from_status: str, to_status: str, reason: str = ""):
        ...
        self._write_lifecycle_event(
            ...
            stage=to_status,
            status="success",
            message=f"State transition: {from_status} -> {to_status}" + (f" ({reason})" if reason else ""),
