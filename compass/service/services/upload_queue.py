...
class UploadQueueManager:
    ...
    def _process_upload(self, task: UploadTask, upload_func: Callable[[str], Any]):
        ...
        with self.semaphore:
            ...
            try:
                self.logger.info(f"Starting upload task {task.task_id} for dataset {task.dataset_id}")
    ...
    def get_queue_size(self) -> int:
        ...
        with self.lock:
            return len([t for t in self.active_uploads.values() if t.status == UploadStatus.PENDING])
    
    def get_active_count(self) -> int:
        """Get number of active uploads."""
        with self.lock:
            return len([t for t in self.active_uploads.values() if t.status == UploadStatus.PROCESSING])
