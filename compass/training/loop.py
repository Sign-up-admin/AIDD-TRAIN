...
def train_epoch(
    model, loader, optimizer, device, scaler, config, trainer, logger=None, current_stage=0
):
    """
            ...
            if (i + 1) % grad_accum_steps == 0 or (i + 1) == len(loader):
                ...
                if save_every_n_batches > 0:
                    ...
                    if (
                        ...
                        checkpoint_filename = (
                            f"checkpoint_epoch_{epoch}_step_{optimizer_step_idx}.pth.tar"
                        )
                        save_checkpoint(
                            checkpoint_data, checkpoint_dir, checkpoint_filename, logger
                        )
