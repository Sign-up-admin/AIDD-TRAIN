...
class RegistryClient:
    ...
    def register_service(
        ...
        try:
            ...
            data = response.json()
            service_id: str = str(data["service_id"])
    ...
    def send_heartbeat(self, service_id: str) -> bool:
        ...
        try:
            response = self.session.post(
                f"{self.registry_url}/api/v1/services/{service_id}/heartbeat", timeout=self.timeout
            )
            response.raise_for_status()
            return True
    ...
    def deregister_service(self, service_id: str) -> bool:
        ...
        try:
            response = self.session.delete(
                f"{self.registry_url}/api/v1/services/{service_id}", timeout=self.timeout
            )
            response.raise_for_status()
    ...
    def discover_services(
        self, service_name: Optional[str] = None, status_filter: Optional[str] = None
    ) -> List[ServiceInfo]:
        """
        ...
        try:
            ...
            response = self.session.get(
                f"{self.registry_url}/api/v1/services", params=params, timeout=self.timeout
            )
            response.raise_for_status()
            data = response.json()
    ...
    def check_registry_health(self) -> bool:
        ...
        """
        try:
            response = self.session.get(f"{self.registry_url}/health", timeout=self.timeout)
